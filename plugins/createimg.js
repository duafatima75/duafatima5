...
cmd({
    pattern: "createimg",
    desc: "Generate AI images from text with FATIMA-MD style",
    category: "ai",
    react: "🎨",
    filename: __filename
},
    ...
    try {
        if (!q) {
            return reply(
                ...
                `> 📌 *Example:* \`${usedPrefix + command} a beautiful girl\`\n` +
                `> ⚡ *Version:* \`12.00\``
            );
        }

        await conn.sendMessage(from, { react: { text: "⏳", key: mek.key } });
        await reply("🎨 *Sedang generate gambar, kripya intezar karein...*");

        const api = new CreateImgAPI();
        const result = await api.generate(q);
...
> 👑 *Powered by ꜰᴀᴛɪᴍᴀ-ᴍᴅ*`
        ...
        console.error("CreateImg Command Error:", e);
        await conn.sendMessage(from, { react: { text: "❌", key: mek.key } });
